    ...
    return function (id, tokens, pathToTwig) {
        ...
        var processDependency = function (token) {
            ...
            if (token.value.indexOf(utils.HASH_PREFIX) === 0) {
                ...
                //check and replace namespaces in filename
                var match = token.value.match(namespaceRegex);
                var namespaces = options.namespaces;
                var normalizedTokenValue;
                if(match && (match[2] in namespaces)) {                  
                    normalizedTokenValue = token.value.replace(match[1],namespaces[match[2]]);
                    token.value = path.relative(path.dirname(resourcePath),normalizedTokenValue);
                    if(token.value.indexOf("../") != 0 || token.value.indexOf("./") != 0) token.value = "./" + token.value;
                }
                else {
